using Distributed
@everywhere begin
    using CellListMap.PeriodicSystems, StaticArrays, Random, Agents, Distributions, Plots, StatsPlots, DataFrames, StatProfilerHTML, CSV,  GeometryBasics, Base.Iterators: accumulate
    ...
    Base.@kwdef mutable struct Parameters
        ...
        blues::Integer
        relaxation::Bool
        results::Vector{Vector{Union{Integer,AbstractFloat}}}
    ...
    function initialize_model(;
        ...
        properties = Parameters(
            ...
            0,
            false,
            Vector{Vector{Union{Integer,AbstractFloat}}}(),
            0,
        ...
        growth_rates::Vector{<: AbstractFloat}=rand(Uniform(0.9,1.1),number_of_particles).*(π*(min_radius^2))
        masss::Vector{<: AbstractFloat}=(radiuss.^2)*π
        model.packing_fraction=sum(masses)/model.area
        ...
        for id in 1:carrying_capacity
            if id<=number_of_particles
                ...
                radius=model.min_radius
                cur_mass=model.min_mass
                cur_gr=growth_rates[begin]
                color= :blue 
                cur_k=k_r
                cur_pos=Tuple(positions[begin])
                isPresent=false 
            end
            add_agent!(
                MakeParticle(
    ...
    function grow_model!(model::ABM)
        ...
        model.packing_fraction=(sum(mass_change)+model.packing_fraction*model.area)/model.area
    end
    ...
    function agents_parallel(model::ABM)
        ...
        activation_order::Vector{Int64} = schedule(model)
        futures::SharedVector{Task} = SharedVector{Task}(length(activation_order))
        @sync for (idx, id) in enumerate(activation_order)
            # Extract the data for the current agent
            agent = model.agents[id]  # Assuming you have a way to get the agent by its ID
            max_radius = model.max_radius
            relaxation = model.relaxation
            periodic = model.periodic
            dt = model.dt
            max_mass = model.max_mass
            wall_constant = model.wall_constant
            sides = model.sides
            cc_limited=model.cc_limited
            # Spawn the task with the data
            futures[idx] = Threads.@spawn agents_parallel_activate(agent, max_radius, relaxation, 
                periodic, dt, max_mass, wall_constant, sides, cc_limited,model)
        end
        final = agents_parallel_result(futures,activation_order)
        return final
    end
    ...
    function agent_parallel_result(futures::SharedVector{Task},activation_order::Vector{Int64})
        # Initialize result
        ...
        for fut in futures
            vec = fetch(fut)
            append!(mass_change, vec[1])
            append!(new_r, vec[2])
            append!(kill_list, vec[3])
            append!(divide_list, vec[4])
        end
        filter!(x -> x ≠ 0, kill_list)
        filter!(x -> x ≠ 0, divide_list)
        return [mass_change, new_r ,kill_list, divide_list, activation_order]
    end
    ...
    function remove_cell!(cell::Particle, model::ABM)
        ...
        model.number_of_particles -=1 
        model.packing_fraction = (model.packing_fraction*model.area - cell.mass)/model.area
        if cell.color==:blue
            model.blues-=1
        else
            model.reds-=1
